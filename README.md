...
The subroutine to do a Wobbler-Calibration is found in the [Tilt_Calibration](src/data_collection/Tilt_Calibration) folder.
...
\end{array}$$
 ...
 The phase shift $\phi_n$ of the hologram carrier frequency of each hologram has to be measured by the <kbd>Measure stack</kbd> of [TiltSeriesUI.s](src/data_collection/Tilt_Series_UI_v1p0.s). The resulting line profile will be used as input for the next step. 
  
  
  
  ### Reconstruction Matrix
  [ReconPS_Holo_Matrix.s](src/data_evaluation/reconstruction/ReconPS_Holo_Matrix.py)<br>
   [ReconPS_Holo_Matrix_with_Fresnel.s](src/data_evaluation/reconstruction/ReconPS_Holo_Matrix_FresnelDiff.py)<br>
  <b>Input:</b> <ul>
...
We have implemented the Taylor series reonstruction as digital Micrograph script [ReconPS_Holo_Matrix_with_Fresnel.py](src/data_evaluation/reconstruction/ReconPS_Holo_Matrix_FresnelDiff.py).
  ...
  [ReconPS_Holo_Image_FresnelDiff.s](src/data_evaluation/reconstruction/ReconPS_Holo_Image_FresnelDiff.s)<br>
<b>Input:</b> <ul>
<li>1D image containing the $\phi_n$ of each hologram</li>
...
<li>tilt series model prediction $\hat{I}(n)$ as 3D image cube</li>
 ...
 Amplitude $A(x,y)$ and phase $\Theta(x,y)$ of the exit wavefunction with respect to the reference can be calculated from the reconstructed images by<br>
...
All scripts are distributed in the hope that they will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
